import { VideoMedium } from "../factories/MediaFactory.js";
...
export function launchLightBox(currentMediumId) {
...
export function closeLightBox() {
	...
	const lightBoxViewItems = Array.from(
		document.querySelectorAll(".light-box-view .built-medium-container")
	);
	lightBoxViewItems
...
export function loadLightBox(media) {
	media.forEach(medium => {
		//DOM element
		const lightBoxView = document.querySelector(
			"#light-box-modal .light-box-view"
		);
		...
		if (medium instanceof VideoMedium) {
			...
			altP.innerText =
				"Votre navigateur ne peut pas lire ce type de contenu";
...
function displayInLightBox(mediumId) {
	...
	lightBoxViewItems.find(
		item => item.getAttribute("data-id") == mediumId
	).style.display = "block";
}
...
function changeLightBoxMedium(direction) {
	...
	const lightBoxViewItems = Array.from(
		lightBox.querySelectorAll(".light-box-view .built-medium-container")
	);
	const arrayMaxIndex = lightBoxViewItems.length - 1;

	//Find current displayed item then hide it and display another one
	const currentDisplayedItem = lightBoxViewItems.find(
...
//Display previous or next medium controls
...
lightBoxNextBtn.addEventListener("click", () => changeLightBoxMedium("next"));
