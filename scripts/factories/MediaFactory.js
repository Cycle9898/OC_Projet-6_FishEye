...
export class MediaFactory {
	constructor(medium, photographer) {
		if (Object.hasOwn(medium, "image")) {
			return new ImageMedium(medium, photographer);
		} else if (Object.hasOwn(medium, "video")) {
...
class ImageMedium {
	...
	get thumbnailImage() {
		const fileName =
			this._image.slice(0, this._image.lastIndexOf(".")) +
			"_thumbnail.jpg";
...
export class VideoMedium {
	...
	get thumbnailImage() {
		const fileName =
			this._video.slice(0, this._video.lastIndexOf(".")) +
			"_thumbnail.jpg";
		return `assets/media/${this.photographerName}_${this.photographerId}/${fileName}`;
	}
