...
//Launch contact modal and add event on the corresponding button
...
firstFocusOnModal.addEventListener("click", closeModal);
...
formInputs.forEach(formInput =>
	formInput.addEventListener("input", () => checkForm(formInput))
);

//Check all inputs and simulate sending data + add event listener
...
function sendForm(event) {
	...
	formInputs.forEach(input =>
		allDDataErrorVisibleAttributes.push(
			input.getAttribute("data-error-visible")
		)
	);

	if (
		allDDataErrorVisibleAttributes.every(attribute => attribute === "false")
	) {
		//simulate data sending to the server and display confirmation message
		formInputs.forEach(input =>
			console.log(
				input.parentNode.querySelector("label").innerText +
					": " +
					input.value
			)
		);
...
modalForm.addEventListener("submit", event => sendForm(event));
