import { launchLightBox, loadLightBox } from "../utils/lightBoxModal.js";
...
function displayPhotographerData(photographer) {
	...
	h1Name.innerText = photographer.name;
	...
	photographerHeader.appendChild(imgPhotographer);
...
function displayPhotographerMedia(media) {
	...
	media.forEach(medium => {
		...
		imgDiv.setAttribute(
			"aria-label",
			`Cliquez ou appuyez sur "Entrée" pour ${medium.mediaReadLabel}`
		);
		...
		const likesNumberHandling = () => {
			...
			if (heartIcon.classList.contains("liked")) {
				...
				totalLikesCounter.innerText =
					Number(totalLikesCounter.innerText) - 1;
			} else {
				heartIcon.classList.add("liked");
				medium._likes += 1;
				span.innerText = medium.likes;
...
function displayTotalLikesNumber(media) {
	...
	totalLikesCounter.innerText = media
		.map(medium => medium.likes)
		.reduce(
			(totalLikesNumber, currentLikesNumber) =>
				totalLikesNumber + currentLikesNumber
		);
}
...
async function init() {
	...
	const photographerData = photographersData.photographers.find(
	...
	const currentPhotographerMedia = photographersData.media.filter(
		media => media.photographerId === photographerId
	);

	//Use MediaFactory to create medium objects with currentPhotographerMedia
	const builtMedia = currentPhotographerMedia.map(
		medium => new MediaFactory(medium, photographer)
	);
